...
function EditFormComponent({ IDPersonneService, refreshData }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoadingInit(true);
      try {
        const [personRes, gradesRes, addrRes, servicesRes, fonctionsRes] =
          await Promise.all([
        ...
        const finalGradeId = gradeFound?.IDWWGrade ?? gradeFound?.WWGradeID ?? gradeFound?.IdWWGrade ?? "";

        const serviceIdDirect = p.ServiceID ?? p.IDService ?? p.IdService ?? "";
        const serviceFound =
          servicesData.find((s) => String(s.IDService) === String(serviceIdDirect)) ||
          servicesData.find((s) =>
        ...
        const finalFonctionId = fonctionFound?.IDFonction ?? fonctionFound?.IdFonction ?? "";

        const codeIdDirect = p.CodeID ?? p.IDCode ?? p.Idcode ?? "";
        const codesSource = fonctionFound?.Codes || [];
        const codeFound =
          codesSource.find((c) => String(c.Idcode ?? c.IDCode) === String(codeIdDirect)) ||
          codesSource.find((c) =>
            clean(c.NomCode) === clean(p.NomCode) || clean(c.Code) === clean(p.Code),
          ) || null;
  ...
  return (
    <>
      <IconButton size="small" title="Editer" onClick={handleOpen} sx={{ ml: 0.5 }}>
            ...
            <Stack spacing={2}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6}>
                  <TextField label="Nom" value={form.NomPersonne}
                    onChange={(e) => setField("NomPersonne", e.target.value)}
                    fullWidth size="small" />
                </Grid>

                <Grid item xs={12} sm={6}>
                  <TextField label="Prenom" value={form.PrenomPersonne}
                    onChange={(e) => setField("PrenomPersonne", e.target.value)}
                    fullWidth size="small" />
                </Grid>

                <Grid item xs={12} sm={6}>
                  <TextField label="Telephone" value={form.TelPro}
                ...
                <Grid item xs={12} sm={6}>
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DatePicker label="Date d'entree" value={form.DateEntreeDate}
                ...
                <Grid item xs={12} sm={6}>
                  <Autocomplete size="small" options={grades}
                    getOptionLabel={(option) => option?.NomWWGradeFr || option?.NomWWGradeNl || ""}
                    value={grades.find((g) =>
                      String(g.IDWWGrade ?? g.WWGradeID ?? g.IdWWGrade) === String(form.WWGradeID)
                    ) || null}
                    isOptionEqualToValue={(option, value) =>
                      String(option.IDWWGrade ?? option.WWGradeID ?? option.IdWWGrade) ===
                      String(value.IDWWGrade ?? value.WWGradeID ?? value.IdWWGrade)
                    }
                    onChange={(e, nv) =>
                      setField("WWGradeID", nv ? (nv.IDWWGrade ?? nv.WWGradeID ?? nv.IdWWGrade) : "")
                    }
                    renderOption={(props, option) => (
                      <li {...props} key={`grade-${option.IDWWGrade ?? option.WWGradeID ?? option.IdWWGrade}`}>
                        {option?.NomWWGradeFr || option?.NomWWGradeNl || "Sans libelle"}
                      </li>
                    )}
                ...
                <Grid item xs={12} sm={6}>
                  <Autocomplete size="small" options={addresses}
                    ...
                    value={addresses.find((a) => String(a.IDAdresse) === String(form.AdresseID)) || null}
                    isOptionEqualToValue={(option, value) =>
                      String(option.IDAdresse) === String(value.IDAdresse)
                    }
                    onChange={(e, nv) => setField("AdresseID", nv ? nv.IDAdresse : "")}
                    renderOption={(props, option) => (
                      <li {...props} key={`adresse-${option.IDAdresse}`}>
                        {option?.AdresseComplete || "Sans libelle"}
                      </li>
                    )}
                    renderInput={(params) => <TextField {...params} label="Adresse" fullWidth />}
                  />
                </Grid>

                <Grid item xs={12} sm={6}>
                  <Autocomplete size="small" options={services}
                    getOptionLabel={(option) => option?.NomServiceFr || option?.NomServiceNl || ""}
                    value={services.find((s) => String(s.IDService) === String(form.ServiceID)) || null}
                    isOptionEqualToValue={(option, value) =>
                      String(option.IDService) === String(value.IDService)
                    }
                    onChange={(e, nv) => setField("ServiceID", nv ? nv.IDService : "")}
                    renderOption={(props, option) => (
                      <li {...props} key={`service-${option.IDService}`}>
                        {option?.NomServiceFr || option?.NomServiceNl || "Sans libelle"}
                      </li>
                    )}
                  ...
                  {selectedServiceLabel ? (
                    <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: "block" }}>
                      Service selectionne : <b>{selectedServiceLabel}</b>
                    </Typography>
                  ) : null}
                </Grid>
                ...
                <Grid item xs={12} sm={6}>
                  <Autocomplete size="small" options={fonctions}
                    getOptionLabel={(option) => option?.NomFonctionFr || option?.NomFonctionNl || ""}
                    value={fonctions.find((f) =>
                    ...
                    renderOption={(props, option) => (
                      <li {...props} key={`fonction-${option.IDFonction ?? option.IdFonction}`}>
                        {option?.NomFonctionFr || option?.NomFonctionNl || "Sans libelle"}
                      </li>
                    )}
                    renderInput={(params) => <TextField {...params} label="Fonction" fullWidth />}
                  />
                </Grid>
                ...
                <Grid item xs={12} sm={6}>
                  <Autocomplete size="small" options={codesDisponibles}
                    ...
                    value={codesDisponibles.find((c) =>
                      String(c.Idcode ?? c.IDCode) === String(form.CodeID)
                    ) || null}
                    isOptionEqualToValue={(option, value) =>
                      String(option.Idcode ?? option.IDCode) === String(value.Idcode ?? value.IDCode)
                    }
                    onChange={(e, nv) => setField("CodeID", nv ? (nv.Idcode ?? nv.IDCode) : "")}
                    renderOption={(props, option) => (
                      <li {...props} key={`code-${option.Idcode ?? option.IDCode}`}>
                        {option?.NomCode || "Sans libelle"}
                      </li>
                    )}
                    renderInput={(params) => <TextField {...params} label="Code fonction" fullWidth />}
                  />
                </Grid>

                <Grid item xs={12} sm={6}>
                  <FormControl>
                    <Typography variant="body2" color="text.secondary" mb={1}>Personnel</Typography>
                ...
                <Grid item xs={12} sm={6}>
                  <FormControl>
                    <Typography variant="body2" color="text.secondary" mb={1}>Langue</Typography>
              ...
              {serviceDetails && (
                <Box sx={{ mt: 1, p: 2, borderRadius: 2, bgcolor: "action.hover" }}>
                  <Typography fontWeight={700} mb={1}>Informations du service</Typography>
                  <Typography variant="body2">
                    <strong>Chef du Service :</strong> {serviceDetails.NomChefService} {serviceDetails.PrenomChefService}
                  </Typography>
                  <Typography variant="body2">
                    <strong>Chef du Departement :</strong> {serviceDetails.NomChefDepartement} {serviceDetails.PrenomChefDepartement}
                  </Typography>
                </Box>
              )}
            </Stack>
